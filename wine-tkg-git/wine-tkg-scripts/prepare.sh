...
_prepare() {
	...
	# esync
	if [ "$_use_esync" = "true" ]; then
	  if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor a87bafc5b92c9f2deaa399e32a8ec42d28f7ea45 HEAD ); then
	...
	if [ "$_lol920_fix" = "true" ] && [ "$_use_staging" = "true" ]; then
	  if git merge-base --is-ancestor 7f144646ffac6f3632d0c39b217dbd433c1154a0 HEAD; then
...
EOM
	...
	if [ "$_EXTERNAL_INSTALL" = "proton" ] && [ "$_unfrog" != "true" ] && ! git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD || ([ "$_protonify" = "true" ] && git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD); then
	  if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor d127a14a7f1941539753f029fb52f407514b1106 HEAD ); then
	    if [ "$_use_staging" = "true" ]; then
	    ...
	    if git merge-base --is-ancestor d327527cec22c1123781e6c5a637032a31698b91 HEAD; then
	      _lastcommit="d127a14"
	      _rpc="1"
	      _stmbits="1"
