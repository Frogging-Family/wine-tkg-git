...
EOM
	...
	if [ "$_proton_fs_hack" = "true" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_use_staging" = "true" ]; then
	    if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 439fb2519091c282094d484ecdae98959fe26142 HEAD ); then
	...
	if [ "$_EXTERNAL_INSTALL" = "proton" ] && [ "$_unfrog" != "true" ] && ! git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD || ([ "$_protonify" = "true" ] && git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD); then
	  if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 0c6d042cc3b1b2cbfb702bb3d199cc7a83e6cbd2 HEAD ); then
	    if [ "$_use_staging" = "true" ]; then
	    ...
	    if git merge-base --is-ancestor bf8c5afef1b3636266bbd2ca686cbebff4d7a849 HEAD; then
	      _lastcommit="0c6d042"
	      _rpc="1"
	      _stmbits="1"
