...
EOM
	...
	if [ "$_proton_fs_hack" = "true" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_use_staging" = "true" ]; then
	    if ( cd "${srcdir}"/"${_stgsrcdir}" && git merge-base --is-ancestor a92ab08688b1e425c887ccb77196bbf681f24be1 HEAD ); then
	      ...
	      if git merge-base --is-ancestor 7f18df46333b5686f2e3622ebc474530e15c6888 HEAD; then
	        _lastcommit="a92ab08"
