...
EOM
	...
	if [ "$_proton_fs_hack" = "true" ] && [ "$_unfrog" != "true" ]; then
	  ...
	  if [ "$_use_staging" = "true" ]; then
	    if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 8a8b5179d0c59d7c4171ad5447ef55689cba86b3 HEAD ); then
	      ...
	      if git merge-base --is-ancestor 656edbb508d51cbe3155d856ee3f2c27a6cd4cba HEAD; then
	        _lastcommit="8a8b517"
	...
	if [ "$_EXTERNAL_INSTALL" = "proton" ] && [ "$_unfrog" != "true" ] && ! git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD || ([ "$_protonify" = "true" ] && git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD); then
	  if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 36ac0c1cfbd298bb8d82a465c00adacdb387599c HEAD ); then
	    if [ "$_use_staging" = "true" ]; then
	    ...
	    if git merge-base --is-ancestor 6b8cbfe3fdc1ab8c6efd55dc51df7c3d738c4a47 HEAD; then
	      _lastcommit="36ac0c1"
	      _rpc="1"
	      _stmbits="1"
